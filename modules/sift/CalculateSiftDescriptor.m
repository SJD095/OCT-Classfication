...
for ii = 1:length(subfolders),
    ...
    if ~strcmp(subname, '.DS_Store')& ~strcmp(subname, '.') & ~strcmp(subname, '..'),
        ...
        for jj = 1:c_num,
            ...
            remX = mod(im_w-patchSize,gridSpacing);%calculate the remained lengths in X-dimension
            offsetX = floor(remX/2)+1;
            remY = mod(im_h-patchSize,gridSpacing);%calculate the remained lengths in X-dimension
            ...
            %             disp(['nclass=',num2str(database.nclass)]);
            %             disp(['jj=',num2str(jj)]);
            %             disp(['path=',database.path{database.nclass,jj}]);
        end;
    end;
end;
